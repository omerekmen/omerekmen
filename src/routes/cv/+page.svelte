...
<svelte:head>
	...
	<meta name="description" content="Curriculum Vitae of {personal.name} — Data Scientist & Software Engineer. Mathematics graduate from Galatasaray University with expertise in ML, Python, SQL, and full-stack development." />
	<meta property="og:type" content="profile" />
	<meta property="og:url" content="https://www.omerekmen.com/cv" />
	<meta property="og:title" content="CV — {personal.name}" />
	<meta property="og:description" content="Interactive CV — Data Scientist & Software Engineer building production-grade data-driven systems." />
...
<div bind:this={pageEl} class="relative min-h-screen bg-bg px-4 py-20 sm:px-6">
	<div class="mx-auto max-w-[900px]">
		...
		<header class="cv-section text-center">
			...
			<!-- Contact row -->
			<div class="mt-5 flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-sm text-text-muted">
				...
				<a href="mailto:omerekmenn@gmail.com" class="flex items-center gap-1.5 transition-colors hover:text-accent">
					<svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
						<rect x="2" y="4" width="20" height="16" rx="2" /><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7" />
					</svg>
					omerekmenn@gmail.com
				</a>
				<a href={personal.website} class="flex items-center gap-1.5 transition-colors hover:text-accent">
					<svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
						<circle cx="12" cy="12" r="10" /><path d="M2 12h20" /><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" />
					</svg>
					omerekmen.com
				</a>
				<a href={personal.linkedin} target="_blank" rel="noopener noreferrer" class="flex items-center gap-1.5 transition-colors hover:text-accent">
					<svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="currentColor">
						<path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
					</svg>
					LinkedIn
				</a>
				<a href={personal.github} target="_blank" rel="noopener noreferrer" class="flex items-center gap-1.5 transition-colors hover:text-accent">
					<svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="currentColor">
		...
		<section class="cv-section mt-12">
			...
			<div class="mt-6 border-l-2 border-accent/20 pl-6">
				<div class="relative">
					...
					<p class="mt-1 text-sm italic text-text-secondary">{education.school} — {education.location}
						<span class="not-italic text-text-muted">| GPA: {education.gpa}</span>
					</p>
					<ul class="mt-4 flex flex-col gap-3">
						{#each education.details as detail (detail.label)}
							<li class="text-sm leading-relaxed text-text-secondary">
								<span class="font-semibold text-text">{detail.label}:</span> {detail.text}
							</li>
						{/each}
					</ul>
				</div>
			...
			<div class="mt-6 flex flex-col gap-10">
				{#each experience as job (job.company)}
					<div class="relative border-l-2 border-accent/20 pl-6">
						...
						<p class="mt-1 text-sm italic text-text-secondary">{job.company} — {job.location}</p>
						<ul class="mt-4 flex flex-col gap-2">
							{#each job.bullets as bullet (bullet)}
								<li class="experience-bullet relative pl-4 text-sm leading-relaxed text-text-secondary">
									<span class="absolute top-[0.55em] left-0 h-1 w-1 rounded-full bg-accent/50"></span>
									{bullet}
								</li>
							{/each}
						</ul>
		...
		<section class="cv-section mt-12">
			...
			<div class="mt-6 flex flex-col gap-6">
				{#each projects as project (project.name)}
					<div class="project-card rounded-xl border border-border-subtle bg-bg-secondary/30 p-5 transition-all duration-200 hover:border-accent/20 hover:bg-bg-secondary/60">
						<div class="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between">
						...
						<div class="mt-4 flex flex-wrap gap-1.5">
							{#each project.stack as tech (tech)}
								<span class="skill-pill rounded-full border border-border-subtle bg-bg-tertiary/50 px-2.5 py-0.5 font-mono text-[10px] font-medium text-text-muted">
									{tech}
								</span>
							{/each}
						</div>
			...
			<div class="mt-6 flex flex-col gap-5">
				{#each skills as group (group.category)}
					<div class="skill-group">
						<h3 class="font-mono text-xs font-semibold tracking-wider text-accent uppercase">{group.category}</h3>
						<div class="mt-2.5 flex flex-wrap gap-2">
							{#each group.items as item (item)}
								<span class="skill-pill rounded-full border border-border-subtle bg-bg-secondary/60 px-3 py-1.5 text-xs font-medium text-text-secondary transition-all duration-200 hover:border-accent/30 hover:bg-accent/5 hover:text-accent">
									{item}
								</span>
							{/each}
						</div>
