	...
	$effect(() => {
		...
		const ctx = gsap.context(() => {
			...
			if (scrollIndicatorEl)
				tl.from(scrollIndicatorEl, { opacity: 0, y: 10, duration: 0.6 }, '-=0.5');
		}, sectionEl);
...
<section bind:this={sectionEl} id="top" class="relative flex min-h-screen flex-col overflow-hidden">
	...
	<!-- Main content area -->
	<div class="relative z-10 flex flex-1 flex-col justify-center gap-8 px-4 py-24 sm:px-8 lg:px-16">
		<!-- Personal card -->
		<div
